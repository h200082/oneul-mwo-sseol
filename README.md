...
> **현재 운영 상태와 제약:** 공개 GitHub Pages 빌드는 Firebase 백엔드, 익명 인증, Firestore 보안 규칙과 저장소 변수를 사용하도록 구성되어 있습니다. 분리된 브라우저 세션의 방 생성·참가·시작·공동 결과 동기화와 미제출 참가자 DNF 처리는 자동 검증했습니다. 다만 3~8대 실기기 네트워크, 모바일 카메라 권한, App Check, 정확한 라운드 진행 지점 복귀, 서버 시간 오프셋 기반 카운트다운 표시와 클라이언트 결과 검증은 후속 QA 범위입니다. 기본 `local` 모드는 동일 출처 멀티탭 개발용입니다.

## 제출 링크

| 항목 | 링크 | 상태 |
|---|---|---|
| 플레이 가능한 웹 빌드 | [GitHub Pages](https://h200082.github.io/oneul-mwo-sseol/) | 공개 운영 배포 |
| 전체 소스 코드 | [GitHub 저장소](https://github.com/h200082/oneul-mwo-sseol) | 공개 저장소 |
| 30~60초 플레이 영상 | 준비 중 | 미완료 |
| 게임 소개 및 설명 문서 | 편집 가능 사전본 생성 | 최종 링크 입력 필요 |
| AI 활용 기술 문서 | 편집 가능 사전본 생성 | 최종 링크 입력 필요 |
| 팀원 롤 기술서 | 개인 참가 | 생략 |
