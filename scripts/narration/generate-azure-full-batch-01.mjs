...
import {
  parseCliArgs as parseExpressiveCliArgs,
...
import {
  ...
  buildFullBatch01Ssml,
  createFullBatch01Manifest,
  createFullBatch01Plan,
  estimateFullBatch01PlannedTiming,
  readFullBatch01ExecutionConfig,
  readFullBatch01PriceCeiling,
  selectFullBatch01Performances,
  summarizeFullBatch01Audio,
  summarizeFullBatch01Cost,
  validateFullBatch01Voices,
} from './azureFullBatch01.mjs'

if (parseCliArgs !== parseExpressiveCliArgs) {
  throw new Error('Azure narration CLI parser mismatch')
}
...
const manifestFileName = 'full-batch-01-manifest.json'

function printHelp() {
  console.log(`Azure MAI narration full batch 01

Usage:
  node scripts/narration/generate-azure-full-batch-01.mjs [--dry-run]
  node scripts/narration/generate-azure-full-batch-01.mjs --execute [--output <directory>]
...
Safety:
  ...
  --execute also requires ${FULL_BATCH_01_PRICE_CEILING_ENV}, set from the
  current official Azure price applicable to the local subscription/region.
  voices/list validates every selected MAI voice and style before any write.
...
async function fetchAvailableVoices(config, performances) {
  const response = await fetch(getEndpoint(config, 'voices/list'), {
    headers: {
      ...
      'User-Agent': 'oneul-mwo-sseol-full-batch-01',
    },
  })
  if (!response.ok) {
  ...
  validateFullBatch01Voices(payload, performances)
}
...
async function synthesizeClip({ config, item, targetPath }) {
  ...
  const response = await fetch(getEndpoint(config, 'v1'), {
    ...
    headers: {
      ...
      'Ocp-Apim-Subscription-Key': config.key,
      'X-Microsoft-OutputFormat': FULL_BATCH_01_OUTPUT_FORMAT,
      'User-Agent': 'oneul-mwo-sseol-full-batch-01',
    },
    body: buildFullBatch01Ssml({
      performance,
...
async function main() {
  ...
  const performances = selectFullBatch01Performances(
    parseNarrationCatalog(source),
  )
  const plan = createFullBatch01Plan(performances)
  const outputDirectory = path.resolve(
    options.outputDir ?? defaultOutputDirectory,
  )
  const dryRunPricing = readFullBatch01PriceCeiling(process.env)
  const dryRunCost = summarizeFullBatch01Cost(
    plan,
  ...
  console.log(`Required region: ${FULL_BATCH_01_REQUIRED_REGION}`)
  console.log(`Batch clips: ${plan.length}`)
  console.log(
    `Listening QA target: ${FULL_BATCH_01_QUALITY_TARGET_SECONDS.minimum}-` +
      `${FULL_BATCH_01_QUALITY_TARGET_SECONDS.maximum}s ` +
      `(hard max ${FULL_BATCH_01_HARD_MAX_SECONDS.toFixed(1)}s)`,
  )
  console.log(
  ...
  for (const item of plan) {
    ...
    console.log(
      `- ${item.relativeFile} [${item.voiceId}; ` +
        ...
        `${performance.setupText} | ${performance.breakMs}ms | ` +
        `${performance.punchText} ` +
  ...
  const config = readFullBatch01ExecutionConfig(process.env)
  const pricing = summarizeFullBatch01Cost(
    plan,
  ...
  console.log('Checking selected MAI voices and required styles...')
  ...
  for (const { item, targetPath } of outputItems) {
    ...
    const summary = summarizeFullBatch01Audio(byteLength)
    ...
    if (!summary.durationWithinHardMaximum) {
      console.warn(
        `QA HARD-MAX warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_HARD_MAX_SECONDS.toFixed(1)}s. ` +
      ...
      console.warn(
        `QA warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_QUALITY_TARGET_SECONDS.minimum}-` +
          `${FULL_BATCH_01_QUALITY_TARGET_SECONDS.maximum}s. ` +
          'The file was kept and no retry was attempted.',
      )
    }
  ...
  const manifest = createFullBatch01Manifest({
    plan,
    audioResults,
...
main().catch((error) => {
  ...
  console.error(`Full narration batch 01 failed: ${message}`)
  process.exitCode = 1
})
