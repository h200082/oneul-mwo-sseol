...
import {
  FULL_BATCH_01_RETAKE_03_HARD_MAX_SECONDS,
  FULL_BATCH_01_RETAKE_03_OUTPUT_FORMAT,
  FULL_BATCH_01_RETAKE_03_PRICE_CEILING_ENV,
  FULL_BATCH_01_RETAKE_03_QUALITY_TARGET_SECONDS,
  FULL_BATCH_01_RETAKE_03_REQUIRED_REGION,
  buildFullBatch01Retake03Ssml,
  createFullBatch01Retake03Manifest,
  createFullBatch01Retake03Plan,
  estimateFullBatch01Retake03PlannedTiming,
  readFullBatch01Retake03ExecutionConfig,
  readFullBatch01Retake03PriceCeiling,
  selectFullBatch01Retake03Performances,
  summarizeFullBatch01Retake03Audio,
  summarizeFullBatch01Retake03Cost,
  validateFullBatch01Retake03Voices,
} from './azureFullBatch01Retake03.mjs'
...
const manifestFileName = 'full-batch-01-retake-03-manifest.json'

function printHelp() {
  console.log(`Azure MAI full batch 01 natural-delivery retake 03

Usage:
  node scripts/narration/generate-azure-full-batch-01-retake-03.mjs [--dry-run]
  node scripts/narration/generate-azure-full-batch-01-retake-03.mjs --execute [--output <directory>]
...
Safety:
  ...
  --execute also requires ${FULL_BATCH_01_RETAKE_03_PRICE_CEILING_ENV}, set from
  ...
  Each clip uses one uninterrupted no-comma full-sentence express-as/prosody block with no explicit break,
...
async function assertNoOutputWillBeOverwritten(targetPaths) {
  ...
  if (existingPaths.length > 0) {
    throw new Error(
      `Refusing to overwrite existing retake 03 output:\n${existingPaths.join('\n')}`,
    )
  }
}
...
async function fetchAvailableVoices(config, performances) {
  const response = await fetch(getEndpoint(config, 'voices/list'), {
    headers: {
      ...
      'User-Agent': 'oneul-mwo-sseol-full-batch-01-retake-03',
    },
  })
  if (!response.ok) {
  ...
  validateFullBatch01Retake03Voices(payload, performances)
}
...
async function synthesizeClip({ config, item, targetPath }) {
  ...
  const response = await fetch(getEndpoint(config, 'v1'), {
    ...
    headers: {
      ...
      'Ocp-Apim-Subscription-Key': config.key,
      'X-Microsoft-OutputFormat': FULL_BATCH_01_RETAKE_03_OUTPUT_FORMAT,
      'User-Agent': 'oneul-mwo-sseol-full-batch-01-retake-03',
    },
    body: buildFullBatch01Retake03Ssml({
      performance,
...
async function main() {
  ...
  const performances = selectFullBatch01Retake03Performances(
    parseNarrationCatalog(source),
  )
  const plan = createFullBatch01Retake03Plan(performances)
  const outputDirectory = path.resolve(
    options.outputDir ?? defaultOutputDirectory,
  )
  const dryRunPricing = readFullBatch01Retake03PriceCeiling(process.env)
  const dryRunCost = summarizeFullBatch01Retake03Cost(
    plan,
  ...
  console.log(`Required region: ${FULL_BATCH_01_RETAKE_03_REQUIRED_REGION}`)
  console.log(`Retake clips: ${plan.length}`)
  console.log(
    `Listening QA target: ${FULL_BATCH_01_RETAKE_03_QUALITY_TARGET_SECONDS.minimum}-` +
      `${FULL_BATCH_01_RETAKE_03_QUALITY_TARGET_SECONDS.maximum}s ` +
      `(hard max ${FULL_BATCH_01_RETAKE_03_HARD_MAX_SECONDS.toFixed(1)}s)`,
  )
  console.log(
  ...
  const config = readFullBatch01Retake03ExecutionConfig(process.env)
  const pricing = summarizeFullBatch01Retake03Cost(
    plan,
  ...
  for (const { item, targetPath } of outputItems) {
    ...
    const summary = summarizeFullBatch01Retake03Audio(byteLength)
    ...
    if (!summary.durationWithinHardMaximum) {
      console.warn(
        `QA HARD-MAX warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_RETAKE_03_HARD_MAX_SECONDS.toFixed(1)}s. ` +
      ...
      console.warn(
        `QA warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_RETAKE_03_QUALITY_TARGET_SECONDS.minimum}-` +
          `${FULL_BATCH_01_RETAKE_03_QUALITY_TARGET_SECONDS.maximum}s. ` +
          'The file was kept and no retry was attempted.',
      )
    }
  ...
  const manifest = createFullBatch01Retake03Manifest({
    plan,
    audioResults,
...
main().catch((error) => {
  ...
  console.error(`Full batch 01 retake 03 failed: ${message}`)
  process.exitCode = 1
})
