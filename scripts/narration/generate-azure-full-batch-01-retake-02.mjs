...
import {
  FULL_BATCH_01_RETAKE_02_HARD_MAX_SECONDS,
  FULL_BATCH_01_RETAKE_02_OUTPUT_FORMAT,
  FULL_BATCH_01_RETAKE_02_PRICE_CEILING_ENV,
  FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS,
  FULL_BATCH_01_RETAKE_02_REQUIRED_REGION,
  buildFullBatch01Retake02Ssml,
  createFullBatch01Retake02Manifest,
  createFullBatch01Retake02Plan,
  estimateFullBatch01Retake02PlannedTiming,
  readFullBatch01Retake02ExecutionConfig,
  readFullBatch01Retake02PriceCeiling,
  selectFullBatch01Retake02Performances,
  summarizeFullBatch01Retake02Audio,
  summarizeFullBatch01Retake02Cost,
  validateFullBatch01Retake02Voices,
} from './azureFullBatch01Retake02.mjs'
...
const defaultOutputDirectory = path.join(
  ...
  'tmp',
  'narration-preview',
  'full-batch-01-retake-02',
)
const manifestFileName = 'full-batch-01-retake-02-manifest.json'

function printHelp() {
  console.log(`Azure MAI full batch 01 natural-delivery retake 02

Usage:
  node scripts/narration/generate-azure-full-batch-01-retake-02.mjs [--dry-run]
  node scripts/narration/generate-azure-full-batch-01-retake-02.mjs --execute [--output <directory>]
...
Safety:
  ...
  --execute also requires ${FULL_BATCH_01_RETAKE_02_PRICE_CEILING_ENV}, set from
  ...
  Each clip uses one full-sentence express-as/prosody block with no explicit break,
...
async function assertNoOutputWillBeOverwritten(targetPaths) {
  ...
  if (existingPaths.length > 0) {
    throw new Error(
      `Refusing to overwrite existing retake 02 output:\n${existingPaths.join('\n')}`,
    )
  }
}
...
async function fetchAvailableVoices(config, performances) {
  const response = await fetch(getEndpoint(config, 'voices/list'), {
    headers: {
      ...
      'User-Agent': 'oneul-mwo-sseol-full-batch-01-retake-02',
    },
  })
  if (!response.ok) {
  ...
  validateFullBatch01Retake02Voices(payload, performances)
}
...
async function synthesizeClip({ config, item, targetPath }) {
  ...
  const response = await fetch(getEndpoint(config, 'v1'), {
    ...
    headers: {
      ...
      'Ocp-Apim-Subscription-Key': config.key,
      'X-Microsoft-OutputFormat': FULL_BATCH_01_RETAKE_02_OUTPUT_FORMAT,
      'User-Agent': 'oneul-mwo-sseol-full-batch-01-retake-02',
    },
    body: buildFullBatch01Retake02Ssml({
      performance,
...
async function main() {
  ...
  const performances = selectFullBatch01Retake02Performances(
    parseNarrationCatalog(source),
  )
  const plan = createFullBatch01Retake02Plan(performances)
  const outputDirectory = path.resolve(
    options.outputDir ?? defaultOutputDirectory,
  )
  const dryRunPricing = readFullBatch01Retake02PriceCeiling(process.env)
  const dryRunCost = summarizeFullBatch01Retake02Cost(
    plan,
  ...
  console.log(`Required region: ${FULL_BATCH_01_RETAKE_02_REQUIRED_REGION}`)
  console.log(`Retake clips: ${plan.length}`)
  console.log(
    `Listening QA target: ${FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS.minimum}-` +
      `${FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS.maximum}s ` +
      `(hard max ${FULL_BATCH_01_RETAKE_02_HARD_MAX_SECONDS.toFixed(1)}s)`,
  )
  console.log(
  ...
  const config = readFullBatch01Retake02ExecutionConfig(process.env)
  const pricing = summarizeFullBatch01Retake02Cost(
    plan,
  ...
  for (const { item, targetPath } of outputItems) {
    ...
    const summary = summarizeFullBatch01Retake02Audio(byteLength)
    ...
    if (!summary.durationWithinHardMaximum) {
      console.warn(
        `QA HARD-MAX warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_RETAKE_02_HARD_MAX_SECONDS.toFixed(1)}s. ` +
      ...
      console.warn(
        `QA warning: ${item.relativeFile} is approximately ` +
          ...
          `${FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS.minimum}-` +
          `${FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS.maximum}s. ` +
          'The file was kept and no retry was attempted.',
      )
    }
  ...
  const manifest = createFullBatch01Retake02Manifest({
    plan,
    audioResults,
...
main().catch((error) => {
  ...
  console.error(`Full batch 01 retake 02 failed: ${message}`)
  process.exitCode = 1
})
