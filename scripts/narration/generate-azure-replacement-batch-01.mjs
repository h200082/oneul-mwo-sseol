...
import {
  REPLACEMENT_BATCH_01_HARD_MAX_SECONDS,
  REPLACEMENT_BATCH_01_OUTPUT_FORMAT,
  REPLACEMENT_BATCH_01_PRICE_CEILING_ENV,
  REPLACEMENT_BATCH_01_QUALITY_TARGET_SECONDS,
  REPLACEMENT_BATCH_01_REQUIRED_REGION,
  buildReplacementBatch01Ssml,
  createReplacementBatch01Manifest,
  createReplacementBatch01Plan,
  estimateReplacementBatch01PlannedTiming,
  readReplacementBatch01ExecutionConfig,
  readReplacementBatch01PriceCeiling,
  selectReplacementBatch01Performances,
  summarizeReplacementBatch01Audio,
  summarizeReplacementBatch01Cost,
  validateReplacementBatch01Voices,
} from './azureReplacementBatch01.mjs'
...
const defaultOutputDirectory = path.join(
  ...
  'tmp',
  'narration-preview',
  'replacement-batch-01',
)
const manifestFileName = 'replacement-batch-01-manifest.json'

function printHelp() {
  console.log(`Azure MAI narration replacement batch 01

Usage:
  node scripts/narration/generate-azure-replacement-batch-01.mjs [--dry-run]
  node scripts/narration/generate-azure-replacement-batch-01.mjs --execute [--output <directory>]
...
Safety:
  ...
  --execute also requires ${REPLACEMENT_BATCH_01_PRICE_CEILING_ENV}, set from
  ...
  Six synthesis requests are made once each, without retries.
...
async function assertNoOutputWillBeOverwritten(targetPaths) {
  ...
  if (existingPaths.length > 0) {
    throw new Error(
      `Refusing to overwrite existing replacement batch 01 output:\n${existingPaths.join('\n')}`,
    )
  }
}
...
async function fetchAvailableVoices(config, performances) {
  const response = await fetch(getEndpoint(config, 'voices/list'), {
    headers: {
      ...
      'User-Agent': 'oneul-mwo-sseol-replacement-batch-01',
    },
  })
  if (!response.ok) {
  ...
  validateReplacementBatch01Voices(payload, performances)
}
...
async function synthesizeClip({ config, item, targetPath }) {
  ...
  const response = await fetch(getEndpoint(config, 'v1'), {
    ...
    headers: {
      ...
      'Ocp-Apim-Subscription-Key': config.key,
      'X-Microsoft-OutputFormat': REPLACEMENT_BATCH_01_OUTPUT_FORMAT,
      'User-Agent': 'oneul-mwo-sseol-replacement-batch-01',
    },
    body: buildReplacementBatch01Ssml({
      performance,
...
async function main() {
  ...
  const performances = selectReplacementBatch01Performances(
    parseNarrationCatalog(source),
  )
  const plan = createReplacementBatch01Plan(performances)
  const outputDirectory = path.resolve(
    options.outputDir ?? defaultOutputDirectory,
  )
  const dryRunPricing = readReplacementBatch01PriceCeiling(process.env)
  const dryRunCost = summarizeReplacementBatch01Cost(
    plan,
  ...
  console.log(`Required region: ${REPLACEMENT_BATCH_01_REQUIRED_REGION}`)
  console.log(`Batch clips: ${plan.length}`)
  console.log(
    `Listening QA target: ${REPLACEMENT_BATCH_01_QUALITY_TARGET_SECONDS.minimum}-` +
      `${REPLACEMENT_BATCH_01_QUALITY_TARGET_SECONDS.maximum}s ` +
      `(hard max ${REPLACEMENT_BATCH_01_HARD_MAX_SECONDS.toFixed(1)}s)`,
  )
  console.log(
  ...
  const config = readReplacementBatch01ExecutionConfig(process.env)
  const pricing = summarizeReplacementBatch01Cost(
    plan,
  ...
  for (const { item, targetPath } of outputItems) {
    ...
    const summary = summarizeReplacementBatch01Audio(byteLength)
    ...
    if (!summary.durationWithinHardMaximum) {
      console.warn(
        `QA HARD-MAX warning: ${item.relativeFile} is approximately ` +
          ...
          `${REPLACEMENT_BATCH_01_HARD_MAX_SECONDS.toFixed(1)}s. ` +
      ...
      console.warn(
        `QA warning: ${item.relativeFile} is approximately ` +
          ...
          `${REPLACEMENT_BATCH_01_QUALITY_TARGET_SECONDS.minimum}-` +
          `${REPLACEMENT_BATCH_01_QUALITY_TARGET_SECONDS.maximum}s. ` +
          'The file was kept and no retry was attempted.',
      )
    }
  ...
  const manifest = createReplacementBatch01Manifest({
    plan,
    audioResults,
...
main().catch((error) => {
  ...
  console.error(`Replacement batch 01 failed: ${message}`)
  process.exitCode = 1
})
