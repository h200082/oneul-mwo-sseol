...
export const FULL_BATCH_01_RETAKE_02_MENU_IDS = Object.freeze([
  'seolleongtang',
  'samgyetang',
])

export const FULL_BATCH_01_RETAKE_02_REQUIRED_REGION =
  FULL_BATCH_01_REQUIRED_REGION
export const FULL_BATCH_01_RETAKE_02_OUTPUT_FORMAT =
  FULL_BATCH_01_OUTPUT_FORMAT
export const FULL_BATCH_01_RETAKE_02_PRICE_CEILING_ENV =
  FULL_BATCH_01_PRICE_CEILING_ENV
export const FULL_BATCH_01_RETAKE_02_DEFAULT_PRICE_CEILING =
  FULL_BATCH_01_DEFAULT_PRICE_CEILING
export const FULL_BATCH_01_RETAKE_02_MP3_BYTES_PER_SECOND =
  FULL_BATCH_01_MP3_BYTES_PER_SECOND
export const FULL_BATCH_01_RETAKE_02_SYNTHESIS_REQUESTS_PER_CLIP = 1
export const FULL_BATCH_01_RETAKE_02_RETRY_COUNT = 0
export const FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS =
  FULL_BATCH_01_QUALITY_TARGET_SECONDS
export const FULL_BATCH_01_RETAKE_02_HARD_MAX_SECONDS =
  FULL_BATCH_01_HARD_MAX_SECONDS
...
export const FULL_BATCH_01_RETAKE_02_PERFORMANCES = Object.freeze([
  Object.freeze({
    ...
    pitch: '+0%',
    reviewIntent:
      'Natural full-sentence determined delivery with standard spelling and no forced beat boundary.',
  }),
  Object.freeze({
    menuId: 'samgyetang',
    voiceId: 'junho',
    ...
    pitch: '+0%',
    reviewIntent:
      'Natural full-sentence joyful delivery for the newly approved clear recovery-game metaphor.',
  }),
])
...
function assertPerformance(performance) {
  if (!FULL_BATCH_01_RETAKE_02_MENU_IDS.includes(performance.menuId)) {
    throw new Error(`Unexpected retake 02 menu: ${performance.menuId}`)
  }
  if (performance.voiceId !== 'junho' || !VOICE_BY_ID.has(performance.voiceId)) {
    throw new Error(`Retake 02 voice must be Junho: ${performance.menuId}`)
  }
  if (performance.spokenText !== performance.catalogText) {
    throw new Error(`Retake 02 spoken copy must exactly match catalog: ${performance.menuId}`)
  }
...
function assertExactlyOnePerformancePerMenu(performances) {
  if (performances.length !== FULL_BATCH_01_RETAKE_02_MENU_IDS.length) {
    throw new Error('Full batch 01 retake 02 must contain exactly two performances')
  }
  const ids = performances.map(({ menuId }) => menuId)
  if (new Set(ids).size !== ids.length) {
    throw new Error('Full batch 01 retake 02 menu IDs must be unique')
  }
  if (
    ids.some(
      (menuId, index) => menuId !== FULL_BATCH_01_RETAKE_02_MENU_IDS[index],
    )
  ) {
    throw new Error('Full batch 01 retake 02 menu order changed')
  }
}

export function estimateFullBatch01Retake02PlannedTiming(performance) {
...
export function selectFullBatch01Retake02Performances(catalog) {
  assertExactlyOnePerformancePerMenu(FULL_BATCH_01_RETAKE_02_PERFORMANCES)
  const narrationById = new Map(catalog.map((item) => [item.menuId, item]))
  for (const performance of FULL_BATCH_01_RETAKE_02_PERFORMANCES) {
    assertPerformance(performance)
    ...
    if (current.text !== performance.catalogText) {
      throw new Error(`Retake 02 narration text is stale: ${performance.menuId}`)
    }
    const timing = estimateFullBatch01Retake02PlannedTiming(performance)
    if (
      timing.approxDurationSeconds >
      FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS.maximum
    ) {
      throw new Error(`Planned duration exceeds target: ${performance.menuId}`)
    }
  }
  return FULL_BATCH_01_RETAKE_02_PERFORMANCES
}

export function createFullBatch01Retake02Plan(
  performances = FULL_BATCH_01_RETAKE_02_PERFORMANCES,
) {
...
export function buildFullBatch01Retake02Ssml({
  performance,
  voiceShortName,
}) {
  assertPerformance(performance)
...
export function readFullBatch01Retake02ExecutionConfig(environment) {
...
export function summarizeFullBatch01Retake02Cost(
  ...
  const files = plan.map((item) => {
    const ssml = buildFullBatch01Retake02Ssml({
...
export function summarizeFullBatch01Retake02Audio(byteLength) {
  return summarizeFullBatch01Audio(byteLength)
}

export function createFullBatch01Retake02Manifest({
  plan,
  audioResults,
  region,
  pricing,
  ...
  if (region !== FULL_BATCH_01_RETAKE_02_REQUIRED_REGION) {
    throw new Error('Manifest region does not match full batch 01 retake 02')
  }
  if (audioResults.length !== plan.length) {
    throw new Error('Manifest audio result count does not match the retake 02 plan')
  }
  ...
  const generatedFiles = plan.map((item) => {
    ...
    return {
      ...
      plannedTiming: estimateFullBatch01Retake02PlannedTiming(performance),
      file: item.relativeFile,
      ssmlCharacters: pricing.files.find(
        (file) => file.relativeFile === item.relativeFile,
      )?.ssmlCharacters,
      byteLength: result.byteLength,
      ...summarizeFullBatch01Retake02Audio(result.byteLength),
    }
  })
  return {
    schemaVersion: 1,
    ...
    region,
    outputFormat: FULL_BATCH_01_RETAKE_02_OUTPUT_FORMAT,
    generatedAt,
    sourceCatalog: 'src/data/menuNarrations.ts',
    sourceListeningReview: 'full-batch-01-retake-01',
    copyReview: {
    ...
    pricing: {
      environmentVariable: FULL_BATCH_01_RETAKE_02_PRICE_CEILING_ENV,
      ceilingSource: pricingSource,
      ...pricing,
    },
    quality: {
      targetSeconds: FULL_BATCH_01_RETAKE_02_QUALITY_TARGET_SECONDS,
      hardMaximumSeconds: FULL_BATCH_01_RETAKE_02_HARD_MAX_SECONDS,
    ...
    requests: {
      ...
      synthesisPerClip:
        FULL_BATCH_01_RETAKE_02_SYNTHESIS_REQUESTS_PER_CLIP,
      retries: FULL_BATCH_01_RETAKE_02_RETRY_COUNT,
    },
    generatedFiles,
  }
}
