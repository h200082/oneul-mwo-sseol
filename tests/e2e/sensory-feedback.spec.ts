...
interface GameDebugState {
  readonly activeToken: {
    ...
    readonly judgement: { readonly radius: number }
  } | null
...
interface SensoryDebugWindow extends Window {
  ...
  __NHN_APP__?: {
    getDebugState: () => { sensoryFeedback: SensoryDebugState }
  }
  __NHN_GAME__?: {
    scene: {
...
async function sliceActiveToken(page: Page): Promise<void> {
  ...
  const start = await getCanvasPoint(
    page,
    token.x - token.judgement.radius * 1.5,
    token.y,
  )
  const end = await getCanvasPoint(
    page,
    token.x + token.judgement.radius * 1.5,
    token.y,
  )
...
test('모바일 첫 터치는 pointerup에서 효과음을 잠금 해제한다', async ({
  ...
  await page.getByTestId('solo-start').tap()
  await expect(page.locator('#game-root canvas')).toBeVisible()
  await expect
    .poll(async () => (await readSensoryDebug(page)).audioState)
    .toBe('running')
...
test('진동 API가 없어도 효과음과 게임 진행은 정상 동작한다', async ({
  ...
  expect(sensory.soundOutputCount).toBe(1)
...
test('베기·포획·놓침은 완료 순간에 한 번씩 피드백을 확정한다', async ({
  ...
  const firstCenter = await getCanvasPoint(page, firstToken.x, firstToken.y)
  ...
  const capturePoint = await getCanvasPoint(
    page,
    captureToken.x,
    captureToken.y,
  )
...
test('모바일 길게 누르기 포획은 진동을 한 번만 확정한다', async ({
  ...
  const capturePoint = await getCanvasPoint(page, token.x, token.y)
  const cdp = await page.context().newCDPSession(page)
  const before = await readSensoryDebug(page)
...
test('모바일 홀드에서 드래그로 바꿔도 취소 피드백이 추가되지 않는다', async ({
  ...
  const start = await getCanvasPoint(page, token.x, token.y)
  const drag = await getCanvasPoint(page, token.x + 32, token.y)
  const cdp = await page.context().newCDPSession(page)
