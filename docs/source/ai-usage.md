...
> 작성 기준일: 2026-08-08
> 대상 프로젝트: **오늘 뭐 썰?** 실제 음식 알파 실루엣 베기·대표 음식 11종·모바일 멀티플레이 프로토타입
...
| Codex 내장 ImageGen | 대표 음식 11종의 2D 스티커 이미지 생성 | 2026-08-01 비주얼 수직 슬라이스, 2026-08-08 비원형 확장 |
...
|---|---|---|---|
| 기하 판정 | `src/domain/geometry.ts`, `src/domain/gestureClassifier.ts`, `src/domain/alphaSilhouette.ts`, `src/domain/silhouetteGestureClassifier.ts` | 이미지 없는 메뉴의 원형 폴백, 실제 음식 알파 접촉·가중 면적 분할, 양끝 최대 32px 보정, 이동 대상 접촉 경로와 화면 드래그 방향 분리 | 50:50·편심·오목형·투명 틈·얇은 부속·같은 쪽 선·짧은 탭·접선·루프·offset/scale 경계 테스트 검토 |
| 게임 규칙 | `src/domain/gameRules.ts` | 20개 비복원 추출, 최대 2회 포획, 미포획 메뉴 평균 점수, 공동 순위, 초·중·후반 낙하 속도 곡선 | 확정 기획과 일치하는지 규칙별 테스트 검토 |
| 메뉴 데이터 | `src/data/menus.ts`, `src/data/menuVisuals.ts` | 디저트를 제외한 한국 일상식 50개와 시간대 가중치, 대표 11종 이미지·128px 알파 마스크 매핑, 원본 비율을 보존하는 `contain` 크기 계산 | 중복·개수·가중치·시드 재현성·이미지 알파·모바일 용량·이미지/마스크 폴백 검토 |
| 플레이 화면 | `src/game/scenes/PrototypeScene.ts` | 투명 음식은 실제 알파 판정, 이미지 없는 메뉴는 원형 폴백, 멈춤 없는 드래그, 0.32초 길게 누르기 포획, 진행 링·에너지 끈, 양분 마스크 | 드래그 중 낙하, 투명 모서리 무효, 실제 몸통 성공, 14px 이동 전환, 포획 중 대상 잠금, 절단 방향, `touchcancel` 정리를 검토 |
...
현재 프로토타입에는 제3자 음식·배경 이미지, 사운드, 음성, 폰트를 포함하지 않았다. 대표 음식 11종은 Codex 내장 ImageGen으로 직접 생성하고 투명 WebP로 후처리했으며, 나머지 39종은 Phaser Graphics 폴백을 사용한다. 효과음은 저장된 음원 없이 Web Audio Oscillator와 Gain으로 실행 중 직접 생성하고, 진동은 지원 기기의 표준 Vibration API만 사용한다. `public/favicon.svg`도 프로젝트에서 직접 작성했다.
