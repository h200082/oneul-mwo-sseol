...
> 작성 기준일: 2026-08-03
> 대상 프로젝트: **오늘 뭐 썰?** 형태 독립 제스처·피드백 5차 프로토타입

## 1. 프로젝트 및 AI 활용 개요

- 게임 제목: 오늘 뭐 썰?
- AI 활용 목적: 기획 구체화, 웹 프로젝트 구성, 핵심 알고리즘 구현 보조, 테스트 설계, 제출 문서화
- 핵심 활용 영역: 게임 규칙 정리, 이미지 형태와 분리된 베기·포획 판정, 모바일 입력 취소 처리, 방·대기실 규칙, QR 초대·스캔 흐름, 로컬 멀티탭과 Firebase 실제 기기용 동기화, 공동 결과 집계, 보안 규칙 테스트, 브라우저 테스트, GitHub Pages 배포 준비
...
|---|---|---|---|
| 기하 판정 | `src/domain/geometry.ts`, `src/domain/gestureClassifier.ts` | 경로 닫힘, 단순 폐곡선, 표준 판정 원 전체 감싸기, 선분·원 교차, 원호 면적, 순수 베기·포획 분류 | 50:50·60:40·75:25 베기와 중심만 감싼 좁은 폐곡선·자기 교차·포획 한도 경계 테스트 검토 |
...
| 플레이 화면 | `src/game/scenes/PrototypeScene.ts` | 투명 음식 실루엣, 공통 판정 마스크, 실제 베기 선을 따르는 양분 마스크, 포획 슬롯, 입력 궤적·피드백·취소 처리 | 원형 크롭 없이도 판정이 일정한지, 베기 조각과 마스크가 함께 이동·정리되는지, `touchcancel`이 점수로 기록되지 않는지 검토 |
...
| 브라우저 시각 검사 | 통과 | DOM 홈·방 생성·QR·대기실·카운트다운, 원형 크롭 없는 음식 표시, 실제 베기 각도의 두 조각, 포획 슬롯과 390×844 게임 화면 확인 |
