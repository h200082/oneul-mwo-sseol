...
- 핵심 활용 영역: 게임 규칙 정리, 이미지 형태와 분리된 완화 베기·길게 누르기 포획 판정, 이동 중 입력과 모바일 취소 처리, 절차형 효과음·선택적 진동, 방·대기실 규칙, QR 초대·스캔 흐름, 로컬 멀티탭과 Firebase 실제 기기용 동기화, 공동 결과 집계, 보안 규칙 테스트, 브라우저 테스트, GitHub Pages 배포 준비
...
| 2026-08-08 | Food Action Sticker v2 | 원형 그릇 중심 구도를 줄이고 베기마다 다른 실루엣을 제공 | 닭다리·날개 X형 치킨, 젓가락 면 리프트 라면 v2, 투명 WebP·게임 매핑·축소 가독성 검증 | 생성 결과를 직접 선별하고 112/68/47px로 축소 검토, 중심 판정과 충돌하지 않도록 무게중심·돌출부를 조정 |
...
| 2026-08-05 | 효과음·진동 피드백 | 외부 음원 없이 조작·판정의 손맛을 높이고 모바일 미지원 환경을 안전하게 처리 | 앱 수명주기 단일 AudioContext, 12개 의미 큐, 독립 설정 저장, Android 표준 진동, 홈·HUD 토글 | BGM·AI 밈 음성은 다기기 동시 재생 피로와 권리 검토 때문에 제외하고, 터치 `pointerup` 잠금 해제·백그라운드·결과 전환·미지원 fallback을 자동 검증 |
...
현재 프로토타입에는 제3자 음식·배경 이미지, 사운드, 음성, 폰트를 포함하지 않았다. 대표 음식 5종은 Codex 내장 ImageGen으로 직접 생성하고 투명 WebP로 후처리했으며, 나머지 45종은 Phaser Graphics 폴백을 사용한다. 효과음은 저장된 음원 없이 Web Audio Oscillator와 Gain으로 실행 중 직접 생성하고, 진동은 지원 기기의 표준 Vibration API만 사용한다. `public/favicon.svg`도 프로젝트에서 직접 작성했다.
