...
> 작성 기준일: 2026-07-31
> 대상 프로젝트: **오늘 뭐 썰?** Firebase 방·공동 결과 3차 프로토타입
...
| 게임 규칙 | `src/domain/gameRules.ts` | 20개 비복원 추출, 최대 2회 포획, 미포획 메뉴 평균 점수, 공동 순위 | 확정 기획과 일치하는지 규칙별 테스트 검토 |
| 메뉴 데이터 | `src/data/menus.ts` | 디저트를 제외한 한국 일상식 50개와 시간대 가중치 | 중복·개수·가중치·시드 재현성 검토 |
| 플레이 화면 | `src/game/scenes/PrototypeScene.ts` | 메뉴 낙하, 드래그 궤적, 베기·포획·미스, HUD와 결과 | 미스 처리 때 제스처 타이머·포인터·경로·일시정지·트레일을 초기화하는지 검토 |
...
다만 실제 Firebase Console 프로젝트 생성, 익명 인증·Firestore 활성화, 운영 규칙 배포, GitHub Actions 변수 등록, 배포 URL의 허용 도메인·API 제한·App Check 검토, 실기기 네트워크 QA는 아직 완료하지 않았다. 시작한 방의 새로고침 복귀는 구현했지만 정확한 라운드 진행 지점은 저장하지 않으며, 끝까지 결과를 내지 않은 참가자의 마감·기권 규칙과 서버 시간 오프셋을 이용한 기기 간 카운트다운 보정도 구현하지 않았다. 현재 시작 시각은 각 기기 시계를 기준으로 계산한다. 현재 저장소의 기본값은 `local`이며, 실제 프로젝트 구성 없이 “기기 간 멀티플레이 완료”로 판단하지 않는다. 근거 URL과 상세 판단은 `docs/evidence/ai-prompts/2026-07-31-firebase-shared-results.md`에 기록했다.

## 8. 외부 에셋 및 오픈소스

현재 프로토타입에는 외부 음식 이미지, 배경 이미지, 사운드, 음성, 폰트를 포함하지 않았다. 화면 요소는 Phaser Graphics와 시스템 폰트로 만들었고 `public/favicon.svg`도 프로젝트에서 직접 작성했다.
...
- 현재 외부 콘텐츠 에셋이 없으므로 제3자 이미지·사운드의 재배포 문제는 없다.
...
- 향후 AI 생성 이미지·음성을 추가하면 도구, 주요 입력, 생성일, 사람의 수정, 사용 위치를 작업 즉시 `docs/evidence/`에 기록한다.
