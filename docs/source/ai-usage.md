...
> 작성 기준일: 2026-08-01
> 대상 프로젝트: **오늘 뭐 썰?** Firebase 멀티플레이·음식 비주얼 4차 프로토타입

## 1. 프로젝트 및 AI 활용 개요

- 게임 제목: 오늘 뭐 썰?
- AI 활용 목적: 기획 구체화, 웹 프로젝트 구성, 핵심 알고리즘 구현 보조, 테스트 설계, 제출 문서화
- 핵심 활용 영역: 게임 규칙 정리, 베기·포획 판정, 방·대기실 규칙, QR 초대·스캔 흐름, 로컬 멀티탭과 Firebase 실제 기기용 동기화, 공동 결과 집계, 보안 규칙 테스트, 브라우저 테스트, GitHub Pages 배포 준비
...
|---|---|---|---|
| 기하 판정 | `src/domain/geometry.ts` | 경로 닫힘, 중심 포함, 선분·원 교차, 원호 면적과 베기 정확도 | 50:50·60:40·75:25 사례와 경계값 테스트 검토 |
| 게임 규칙 | `src/domain/gameRules.ts` | 20개 비복원 추출, 최대 2회 포획, 미포획 메뉴 평균 점수, 공동 순위, 초·중·후반 낙하 속도 곡선 | 확정 기획과 일치하는지 규칙별 테스트 검토 |
| 메뉴 데이터 | `src/data/menus.ts`, `src/data/menuVisuals.ts` | 디저트를 제외한 한국 일상식 50개와 시간대 가중치, 대표 5종 이미지 매핑 | 중복·개수·가중치·시드 재현성·이미지 폴백 검토 |
| 플레이 화면 | `src/game/scenes/PrototypeScene.ts` | 메뉴 낙하, 대표 음식 이미지, 드래그 궤적, 베기·포획·미스, HUD와 결과 | 이미지 유무가 원형 판정과 점수에 영향을 주지 않는지, 입력 상태가 정상 초기화되는지 검토 |
...
| 방·대기실 | `src/domain/room.ts`, `src/rooms/` | 8자리 코드, 2~8인, 준비 상태 없음, URL 유지, 동일 `playerId` 멱등 재접속, 시작 후 미제출 게임·제출 완료 결과 복귀, 방장 승계·마지막 이탈 삭제, 공통 덱 시드 | 대기실·게임·결과 대기 중 새로고침 복귀를 E2E로 확인하고 로컬 검증 범위를 명시 |
...
| 공동 결과 | `src/domain/roomResults.ts`, `src/app/AppController.ts` | 전원 결과 대기, 1~8등·공동 순위, 포획 카드, 공동 1등·꼴찌, 최다 중복 메뉴 | 시작 때 잠긴 명단만 집계하고 미포획·동점·겹침 없음 사례를 테스트 |
...
- 목적: 같은 드래그 입력을 열린 베기와 닫힌 포획으로 구분
- 사용 도구: Codex
- 주요 지시: “열린 선이 메뉴를 가로지르면 실제 원 면적 분할 비율로 점수화하고, 닫힌 경로가 메뉴 중심을 포함하면 포획한다.”
- 생성 결과: 선분·원 교차, 원호 면적, 경로 닫힘과 중심 포함 판정
- 사람의 검토: 실제 드래그 궤적의 밖→안→밖 교차 구간을 사용하도록 수정하고, 퇴화·자기 교차 포획을 거부하며 메뉴당 정지 기회를 한 번으로 제한
- 반영 위치: `src/domain/geometry.ts`, `src/game/scenes/PrototypeScene.ts`
...
| `npm run test:firebase-rules` | 통과 | Firestore 에뮬레이터에서 인증·방 상태 전이·필드 제한·결과 불변성의 허용·거부 경계 |
| `npm run test:e2e` | 통과 | 홈, 핵심 제스처, 동일 출처 멀티탭 방 시작·승계, 대기실·게임·결과 대기 중 재접속, 공동 결과 화면 |
| `npm run build` | 통과 | Vite 프로덕션 빌드와 `dist/` 생성 |
| 브라우저 시각 검사 | 통과 | DOM 홈·방 생성·QR·대기실·카운트다운과 390×844 게임 화면 확인 |

자동 검증은 로컬 게이트웨이와 에뮬레이터 안의 보안 규칙을 대상으로 한다. 시작한 방의 새로고침 복귀는 로컬 E2E로 검증했지만, 미제출 플레이어는 라운드 진행 지점을 이어받지 않고 같은 덱을 처음부터 다시 플레이한다. 실제 Firebase 프로젝트의 네트워크 지연·오프라인 복귀, 모바일 카메라 권한, 서로 다른 2~8대 기기의 QR 참가와 결과 동기화는 아직 검증하지 않았다. 결과 미제출자의 제한 시간·기권 처리, 기기 시계 차이를 보정하는 서버 시간 동기화와 클라이언트 결과 검증도 별도 QA·후속 구현 항목이다.
